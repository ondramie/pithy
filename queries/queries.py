...
from RuntimeErrors import StackErrors                       # class of common errors
...
def main():
    ...
    myError = StackErrors()._index                      # error under investigation
    type_error = "index error"                          #"Attribute Error"
    request_timeout = 180
    ...
    for _ in range(1):
        with open("search_results.csv", "a") as output:
            ...
            mytimer = MyTimer()
            query_type = "match"
            res = searches(es, query_type, index, "Body", myError, 10)
